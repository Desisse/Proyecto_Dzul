...
test('filtrado por categoría, subcategoría y selección de producto', async ({ page }) => {
  // Navegar a la página principal
  await page.goto('https://www.cyberpuerta.mx/?gad_source=1&gclid=Cj0KCQiAgJa6BhCOARIsAMiL7V-_2gCMouPwuLdIbXYBJ-BqeLIdH7tlZUZMUs0_uXjMy4gjcRRhOfUaApkSEALw_wcB');
  await page.pause();

  // Localizar el menú lateral (sidebar)
  const sidebar = page.locator('#sidebar');
  await expect(sidebar).toBeVisible({ timeout: 30000 });

  // Localizar y hacer clic en la categoría "Computadoras"
  const categoryLink = sidebar.getByRole('link', { name: 'Computadoras' });
  await expect(categoryLink).toBeVisible({ timeout: 30000 });
  await categoryLink.click();
  console.log('Clic en la categoría "Computadoras".');
  await page.pause();

  // Navegar a la subcategoría "Laptops"
  const subCategoryLink = page.locator('a[title="Laptops"]');
  await expect(subCategoryLink).toBeVisible({ timeout: 30000 });
  await subCategoryLink.click();
  console.log('Clic en la subcategoría "Laptops".');
  await page.pause();

  // Filtrar por "ACER (59)"
  const acerFilter = page.locator('text=ACER (59)');
  await expect(acerFilter).toBeVisible({ timeout: 10000 });
  await acerFilter.click();
  console.log('Filtrado por la marca "ACER (59)".');
  await page.pause();

  // Filtrar por "En existencia (36)"
  const inStockFilter = page.locator('text=En existencia (36)');
  await expect(inStockFilter).toBeVisible({ timeout: 10000 });
  await inStockFilter.click();
  console.log('Filtrado por "En existencia (36)".');
  await page.pause();

  // Filtrar por "AMD Ryzen 3 5000 (1)"
  const inStockCoreFilter = page.locator('text=AMD Ryzen 3 5000 (1)');
  await expect(inStockCoreFilter).toBeVisible({ timeout: 10000 });
  await inStockCoreFilter.click();
  console.log('Filtrado por "AMD Ryzen 3 5000 (1)".');
  await page.pause();

  // Seleccionar el producto específico
  const productLink = page.locator('a#productList-1');
  await expect(productLink).toBeVisible({ timeout: 10000 });
  await productLink.click();
  console.log('Seleccionado el producto: Laptop Acer Aspire 5 A515-45G-R854.');
  await page.pause();
